...
BEGIN
  ...
  ) THEN RAISE EXCEPTION 'Warehouse not found'; END IF;
  ...
  LOOP
    BEGIN
      ...
      IF (_item->>'quantity') IS NULL OR (_item->>'quantity') !~ '^[0-9]+$' THEN
  ...
  LOOP
    ...
    IF _track_inventory THEN
      ...
      ELSE
        ...
        WHERE ws.warehouse_id = _warehouse_id AND ws.product_id = _product_id AND w.user_id = _owner
        FOR UPDATE OF ws;
      END IF;
  ...
  IF _track_inventory THEN
    ...
    LOOP
      IF _warehouse_id IS NULL THEN
        UPDATE public.products SET quantity = quantity - _quantity, updated_at = now()
