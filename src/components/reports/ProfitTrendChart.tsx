...
export function ProfitTrendChart({ days = 30, startDate, endDate }: ProfitTrendChartProps) {
  ...
  const chartConfig = {
    grossProfit: {
      label: t('products.grossProfitLabel'),
      color: 'hsl(142, 76%, 36%)',
    },
    netProfit: {
      label: t('products.netProfitLabel'),
      color: 'hsl(217, 91%, 60%)',
    },
    sales: {
      label: t('products.totalSalesLabel'),
      color: 'hsl(280, 65%, 60%)',
    },
    expenses: {
      label: t('products.expensesLabel'),
      color: 'hsl(0, 84%, 60%)',
    },
  ...
  const chartData = useMemo(() => {
    ...
    return filteredStats
      ...
      .map(stat => ({
        ...stat,
        dateLabel: new Date(stat.date).toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-US', {
          month: 'short',
          day: 'numeric'
        }),
      }));
  }, [days, startDate, endDate, language]);
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t('products.profitTrend')}
          </CardTitle>
          <CardDescription>{t('products.noDataInPeriod')}</CardDescription>
        </CardHeader>
      </Card>
    );
  }
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span className="flex items-center gap-2">
            <TrendingUp className="h-5 w-5" />
            {t('products.profitTrendDays').replace('{days}', String(chartData.length))}
          </span>
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-purple-50 dark:bg-purple-950/30 rounded-lg p-3">
            <p className="text-xs text-muted-foreground">{t('products.totalSalesLabel')}</p>
            <p className="text-lg font-bold text-purple-600 dark:text-purple-400">
              {formatCurrency(summary.totalSales)}
            </p>
          </div>
          <div className="bg-green-50 dark:bg-green-950/30 rounded-lg p-3">
            <p className="text-xs text-muted-foreground">{t('products.grossProfitLabel')}</p>
            <p className="text-lg font-bold text-green-600 dark:text-green-400">
              {formatCurrency(summary.totalGrossProfit)}
            </p>
          </div>
          <div className="bg-red-50 dark:bg-red-950/30 rounded-lg p-3">
            <p className="text-xs text-muted-foreground">{t('products.expensesLabel')}</p>
            <p className="text-lg font-bold text-red-600 dark:text-red-400">
              {formatCurrency(summary.totalExpenses)}
            </p>
          </div>
          <div className="bg-blue-50 dark:bg-blue-950/30 rounded-lg p-3">
            <p className="text-xs text-muted-foreground">{t('products.netProfitLabel')}</p>
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <ChartTooltip
                content={
                  <ChartTooltipContent
                    labelFormatter={(value) => `${t('products.dateLabel')}: ${value}`}
                    formatter={(value, name) => {
                      const labels: Record<string, string> = {
                        grossProfit: t('products.grossProfitLabel'),
                        netProfit: t('products.netProfitLabel'),
                        sales: t('products.totalSalesLabel'),
                        expenses: t('products.expensesLabel'),
                      };
        ...
        <div className="mt-4 pt-4 border-t flex justify-between text-sm text-muted-foreground">
          <span>{t('products.avgDailyProfit')}: <strong className="text-foreground">{formatCurrency(summary.avgDailyProfit)}</strong></span>
          <span>{t('products.profitMargin')}: <strong className="text-foreground">{summary.profitMargin.toFixed(1)}%</strong></span>
        </div>
      </CardContent>
    </Card>
  );
