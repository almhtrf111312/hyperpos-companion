  ...
  return (
    <div className="scanner-ui-overlay fixed inset-0 z-[120] bg-black flex flex-col">
        ...
        <div className="flex items-center gap-3">
          {hasTorch && (
            <button
              type="button"
              onClick={toggleTorch}
              className={`h-11 w-11 rounded-full flex items-center justify-center border ${torchOn ? 'bg-yellow-400/80 border-yellow-300 text-black' : 'bg-white/10 border-white/30 text-white'}`}
            >
      ...
      <div className="px-4 py-3 text-center bg-black/80">
        <p className="text-xs text-white/70">وجّه الكاميرا نحو الباركود</p>
      </div>
