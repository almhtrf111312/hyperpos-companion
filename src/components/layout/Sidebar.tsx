...
const navItems: NavItem[] = [
  { icon: ShoppingCart, translationKey: 'nav.pos', tooltipKey: 'tooltip.nav.pos', path: '/' },
  { icon: LayoutDashboard, translationKey: 'nav.dashboard', tooltipKey: 'tooltip.nav.dashboard', path: '/dashboard', adminOnly: true },
  { icon: FileText, translationKey: 'nav.invoices', tooltipKey: 'tooltip.nav.invoices', path: '/invoices' },
  { icon: Package, translationKey: 'nav.products', tooltipKey: 'tooltip.nav.products', path: '/products', adminOnly: true },
  { icon: Users, translationKey: 'nav.customers', tooltipKey: 'tooltip.nav.customers', path: '/customers' },
  { icon: CreditCard, translationKey: 'nav.debts', tooltipKey: 'tooltip.nav.debts', path: '/debts' },
  { icon: Wrench, translationKey: 'nav.services', tooltipKey: 'tooltip.nav.services', path: '/services' },
  { icon: UserCheck, translationKey: 'nav.partners', tooltipKey: 'tooltip.nav.partners', path: '/partners', adminOnly: true },
  { icon: Receipt, translationKey: 'nav.expenses', tooltipKey: 'tooltip.nav.expenses', path: '/expenses' },
  { icon: Wallet, translationKey: 'nav.cashShifts', tooltipKey: 'tooltip.nav.cashShifts', path: '/cash-shifts' },
  { icon: Package, translationKey: 'nav.warehouses', tooltipKey: 'tooltip.nav.warehouses', path: '/warehouses', adminOnly: true },
  { icon: Package, translationKey: 'nav.stockTransfer', tooltipKey: 'tooltip.nav.stockTransfer', path: '/stock-transfer', adminOnly: true },
  { icon: BarChart3, translationKey: 'nav.reports', tooltipKey: 'tooltip.nav.reports', path: '/reports', adminOnly: true },
  { icon: Palette, translationKey: 'settings.theme', tooltipKey: 'tooltip.nav.appearance', path: '/appearance' },
  { icon: Settings, translationKey: 'nav.settings', tooltipKey: 'tooltip.nav.settings', path: '/settings', adminOnly: true },
];
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-sidebar-border">
          ...
          <div className="flex items-center gap-1">
            ...
            {isMobile ? (
              <AppTooltip tooltipKey="tooltip.sidebar.close">
                <button
                  onClick={onToggle}
              ...
              <AppTooltip tooltipKey="tooltip.sidebar.collapse">
                <button
        ...
        <nav className="flex-1 py-4 px-3 overflow-y-auto">
          <ul className="space-y-1">
            {filteredNavItems.map((item) => {
              ...
              return (
                <li key={item.path}>
                  <AppTooltip tooltipKey={item.tooltipKey} side={isRTL ? 'left' : 'right'}>
                    <NavLink
                      to={item.path}
                      className={cn(
        ...
        <div className={cn(
          ...
          <div className={cn(
            ...
            {(!effectiveCollapsed || isMobile) && (
              <AppTooltip tooltipKey="tooltip.sidebar.logout">
                <button
...
export function MobileMenuTrigger({ onClick }: { onClick: () => void }) {
  const { isRTL } = useLanguage(); // ✅ Use hook for consistency

  return (
    <AppTooltip tooltipKey="tooltip.sidebar.menu">
      <button
        onClick={onClick}
        className={cn(
          "fixed top-4 z-30 w-12 h-12 rounded-xl bg-primary text-primary-foreground flex items-center justify-center shadow-lg glow md:hidden transition-all duration-300",
